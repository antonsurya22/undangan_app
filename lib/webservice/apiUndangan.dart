...
class ApiUndangan {
  final url = "http://192.168.8.115:8080/apiundangan/getListUndangan.php";
  ...
  Future<Undangan?> cekUndangan(String email) async {
    final response = await http
        .get(Uri.parse("http://192.168.8.115:8080/apiundangan/cekUndangan.php?email=$email"));
    if (response.statusCode == 200) {
  ...
  Future<bool> updateKehadiran(Undangan undangan) async {
    final response = await http.get(Uri.parse(
        "http://192.168.8.115:8080/apiundangan/updateKehadiran.php?undangan_id=${undangan.undanganID}"));
    if (response.statusCode == 200) {
      return true;
    } else {
      return false;
  ...
  Future<bool> resetKehadiran() async {
    final response = await http.get(Uri.parse(
        "http://192.168.8.115:8080/apiundangan/resetKehadiran.php"));
    if (response.statusCode == 200) {
      return true;
    } else {
      return false;
